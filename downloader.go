...
func validateInputParams(opts DownloadExecutableOptions) error {
	...
	if opts.InfoPrinter == nil {
		return fmt.Errorf("InfoPrinter can't be nil")
	}
	if opts.WarnPrinter == nil {
		return fmt.Errorf("WarnPrinter can't be nil")
	}
	return nil
}
...
func downloadBinary(opts DownloadExecutableOptions) (string, error) {
	...
	defer cancelFunc()
	runDownloadProgressReporter(ctx, opts.InfoPrinter, destFile.Name(), resp.ContentLength)
...
func runDownloadProgressReporter(
	ctx context.Context, infoPrinter func(string), filePath string, expectedFileSize int64,
) {
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				infoPrinter(fmt.Sprintf("file download: %v%%", stat.Size()/(expectedFileSize/100)))
			case <-ctx.Done():
				return
			}
		}
